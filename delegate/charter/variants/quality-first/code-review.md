...
5. If approved: task moves to `in_approval`. Human gives final approval, or a `reviewer` agent reviews via `task_approve`/`task_reject` MCP tools (when reviewer mode is `ai`), or auto-merge for no-review repos.
