...
3. QA checks out the branch, runs the full test suite, verifies quality.
4. QA runs linting (ruff) and type checking (pyright/mypy) — zero violations allowed.
5. QA verifies test coverage has not decreased.
6. QA merges and sends `MERGED` acknowledgment, or sends `REJECTED` with reason.
