"""Tests for scripts/orchestrator.py — multi-agent spawn/manage logic."""
...
from scripts.mailbox import deliver, Message
...
def _set_pid(tmp_team, agent, pid):
    ...
    state_file = tmp_team / ".standup" / "team" / agent / "state.yaml"
...
class TestCheckAndClearStalePids:
    def test_clears_stale_pid(self, tmp_team):
        ...
        cleared = check_and_clear_stale_pids(tmp_team)
    ...
    def test_logs_event_on_clear(self, tmp_team):
        ...
        check_and_clear_stale_pids(tmp_team)
...
class TestGetAgentsNeedingSpawn:
    def test_no_unread_no_spawn(self, tmp_team):
        assert get_agents_needing_spawn(tmp_team) == []

    def test_unread_triggers_spawn(self, tmp_team):
        _deliver_msg(tmp_team, "alice")
        result = get_agents_needing_spawn(tmp_team)
    ...
    def test_concurrency_limit(self, tmp_team):
        ...
        result = get_agents_needing_spawn(tmp_team, max_concurrent=2)
        assert len(result) <= 2
...
class TestOrchestrateOnce:
    def test_spawns_agent(self, tmp_team):
        ...
        def mock_spawn(root, agent):
            spawned_agents.append(agent)

        result = orchestrate_once(tmp_team, spawn_fn=mock_spawn)
        assert "alice" in result
        assert "alice" in spawned_agents

    def test_no_spawn_if_no_unread(self, tmp_team):
        result = orchestrate_once(tmp_team, spawn_fn=lambda r, a: None)
        assert result == []
    ...
    def test_logs_spawn_event(self, tmp_team):
        _deliver_msg(tmp_team, "alice")
        orchestrate_once(tmp_team, spawn_fn=lambda r, a: None)
        events = get_messages(tmp_team, msg_type="event")
    ...
    def test_handles_spawn_failure(self, tmp_team):
        ...
        def failing_spawn(root, agent):
            raise RuntimeError("spawn failed")

        result = orchestrate_once(tmp_team, spawn_fn=failing_spawn)
